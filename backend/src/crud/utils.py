...
from src.crud.models import OTP, User, Message, Contacts
...
async def get_db_contacts(contacts) -> Contacts | None:
    query = select(Contacts).where(Contacts.contacts == contacts)
...
async def add_db_contacts(contacts: Contacts) -> None:
    async with async_session() as session:
        async with session.begin():
