...
from src.crud.models import OTP, Message, User, Contacts
from src.crud.utils import insert_object, get_user_by_id, update_user, get_db_contacts, add_db_contacts
...
class TGClient:
    ...
    async def __send_message(self, receiver: str, message: str):
        try:
            ...
            num_contacts = InputPhoneContact(
                client_id=0,
                phone=receiver,
                first_name=receiver,
                last_name=receiver
            )
            await self._app(ImportContactsRequest([num_contacts]))
    ...
    async def _add_contact(self, contacts) -> None:
        db_contacts = await get_db_contacts(contacts)

        if db_contacts:
            # if contact in db no need to add again
            return
    ...
    async def send_message(self, receiver: str, message: str):
        await self._add_contact(receiver)
    ...
    # async def on_delete(self, contact): todo
